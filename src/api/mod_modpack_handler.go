...
func ModPackModPortalInstallHandler(w http.ResponseWriter, r *http.Request) {
	...
	_, err = modList.DownloadMod(data.DownloadURL, data.Filename, data.ModName, nil)
	if err != nil {
	...
	for _, datum := range data {
		...
		for _, release := range details.Releases {
			if release.Version.Equals(datum.Version) {
				...
				_, err := modList.DownloadMod(release.DownloadURL, release.FileName, details.Name, nil)
				if err != nil {
